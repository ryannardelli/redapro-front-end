...
//                   {React.createElement(
//                   AVAILABLE_ICONS[activeItem?.icon || menu.icon] || HelpCircle,
//                   { size: 22 }
//                 )}
//                   <div>
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 bg-slate-50 min-h-screen">
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center">
                    ...
                    {isActive ? (
                      <div className="relative flex items-center group/name">
                        <input
                          ...
                          className="font-bold bg-transparent border-b border-blue-200 focus:border-blue-500 outline-none pr-6 w-full max-w-[250px] transition-colors"
                        />
                        <PenLine size={14} className="absolute right-0 text-slate-300 opacity-0 group-hover/name:opacity-100 transition-opacity" />
                      </div>
                    ) : (
                      <p className="font-bold">{menu.name}</p>
                    )}
                ...
                >
                  {isActive ? <CheckCircle2 className="text-blue-500" /> : <Plus />}
                </button>
              </div>

              {isActive && (
                <div className="mt-4 flex gap-2 flex-wrap animate-in fade-in slide-in-from-top-2 duration-300">
                  {(Object.keys(AVAILABLE_ICONS) as IconName[]).map(icon => (
                    <button
                      key={icon}
                      onClick={() => updateIcon(menu.route, icon)}
      ...
      <button
        ...
        disabled={!hasChanges || isSaving}
        className="mt-8 w-full bg-blue-600 py-4 rounded-2xl font-bold text-white
                   hover:bg-blue-500 transition-all disabled:opacity-50 active:scale-[0.98]"
      >
        {isSaving ? "Salvando..." : "Salvar alterações"}
      </button>
    </div>
